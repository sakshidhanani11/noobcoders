...
import json
from ..utils import broadcast_alert, send_sms_if_needed, broadcast_reading # Import broadcast_reading
...
async def ingest_reading(reading: ReadingIn, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    background_tasks.add_task(broadcast_reading, {
        "sensor_type": reading.sensor_type,
        "source": reading.source,
        "values": reading.values,
    ...
    if prob > 0.7:
        ...
        background_tasks.add_task(broadcast_alert, {
            "id": alert.id,
            "type": alert.alert_type, # This 'type' is for the database, not the WS message type
            "severity": alert.severity,
            "message": alert.message,
            "payload": alert.payload,
            "created_at": alert.created_at.isoformat() # Convert datetime to string
        })
